...
export function SchemaConfirmCard() {
  const [target, setTarget] = React.useState("churned")
  return (
    <AgentCard
    ...
    >
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        <Mini label="Rows" value="180" />
        <Mini label="Columns" value="6" />
        <Mini label="Task type" value="Classification" />
        <Mini label="Quality score" value="92 / 100" tone="success" />
      </div>
      ...
      <div className="mt-3 flex items-start gap-2 rounded-lg border border-warning/30 bg-warning-soft/50 px-3 py-2 text-[13px] text-foreground">
        ...
        <span>
          <strong>nps_score</strong> has 8% nulls. The agent can impute with the median or drop affected rows.{" "}
